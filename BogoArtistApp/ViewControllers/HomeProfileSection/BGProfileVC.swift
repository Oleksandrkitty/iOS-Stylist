...
class BGProfileVC: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITextFieldDelegate, UITextViewDelegate {
    ...
    func initialize() {
        ...
        DispatchQueue.main.async {
            ...
            if !self.isFirstTime {
                self.newprofileImageView.sd_setImage(with: URL(string: self.stylist.image), placeholderImage: #imageLiteral(resourceName: "bg_image"), options: .refreshCached)
            }

            self.profileNameTextField.text = self.stylist.firstName + " " + self.stylist.lastName

            if (self.stylist.description == "") {
                self.descriptionTextView.placeholder = "Describe yourself"
            } else {
                self.descriptionTextView.text = self.stylist.description
            }
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        stylist.description = textField.text!
    }
    ...
    @objc func showSpinningWheel(_ notification: NSNotification) {
        ...
        stylist.latitude = String(format: "%f", lattitude as! CVarArg)
        stylist.longitude = String(format: "%f", longTitude as! CVarArg)
        if backFromLocation {
    ...
    @IBAction func cancelButtonAction(_ sender: UIButton) {
        if sender.currentTitle == "Cancel" {
            self.newprofileImageView.sd_setImage(with: URL(string: self.stylist.image), placeholderImage: UIImage.init(named: "bg_image"), options: .refreshCached)
            saveButton.setTitle("Logout", for: .normal)
        } else {
...
//        } else
//            if profileData.profileName.trimWhiteSpace.length < nameMinLength {
