...
class ServiceHelper: NSObject {
    ...
    class func createRequestToUploadMultipleData(dataArray : Array<Dictionary<String,Any>>, method: MethodType, apiName: String, hudType: loadingIndicatorType, completionBlock: @escaping (AnyObject?, Error?, Int) -> Void) ->Void {
        
        var stringURL = webApiBaseURL
        stringURL.append(apiName)
        Debug.log("Request URL =\(stringURL)")
        ...
        let task = URLSession.shared.dataTask(with: request as URLRequest) {
            ...
            if response != nil {
                ...
                Debug.log("responseString = \(dataString!)  === \(String(describing: data))")
    ...
    class private func createMultipleBody(arrValue : Array<Dictionary<String,Any>>,boundary : String) -> Data{
        var body = Data()
        for (_, item) in arrValue.enumerated() {
            for (key, value) in item {
                if (value is String || value is NSString){
                    ...
                    body.append("Content-Disposition: form-data; filename=\"image_\(Int(Date().timeIntervalSince1970)).jpg\"; name=\"\(key)\"\r\n")
                    ...
                    for index in arr! {
                        ...
                        body.append(index)
                        body.append("\r\n")
                        valueArra.append(index)
                    }
                    body.append("--\(boundary)\r\n")
                    body.append("Content-Disposition: form-data; name=\"\(key)\"\r\n\r\n")
                    body.append("\(valueArra)\r\n")
                }
            }
        }
        body.append("--\(boundary)--\r\n")
...
extension URLRequest  {
    ...
    func perform(hudType: loadingIndicatorType, completionBlock: @escaping (AnyObject?, Error?, HTTPURLResponse?) -> Void) -> Void {
        ...
        if (APPDELEGATE.isReachable == false) {
            ...
            let userInfo = [ NSLocalizedDescriptionKey : "Internet connection appears to be offline. Please check your internet connection.",
                             NSLocalizedFailureReasonErrorKey : "Connection Error"]
